...
df_dates = pd.DataFrame(data = df_prices_q['date'].unique(), columns = ['date'] , index=range(len(df_prices_q['date'].unique())))
...
dates = df_prices_q['date'].unique().tolist()
symbol_idx = df_idx_p.symbol.unique().tolist()
dict_idx = list(product(symbol_idx, dates))
dict_masteridx = [{'symbol': i[0], 'date': i[1]} for i in dict_idx]

df_idx_q = q_stocks(dict_masteridx, 10,  db.idx_price)

df_idx_p.to_csv('./data/df_index_prices.csv')
df_idx_all.to_csv('./data/df_index_all.csv')
